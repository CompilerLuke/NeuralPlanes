...
class Unet(nn.Module):
    ...
    def _bench_load_vs_gen_backbone(self, image):
        t0 = time.time()
        x, features = self.forward_backbone(image)
        t1 = time.time()

        print("Encode took", (t1-t0)*1000, "ms")

        torch.save({"x": x, "features": features}, "embeddings.pt")

        t0 = time.time()
        embeddings = torch.load("embeddings.pt")
        t1 = time.time()
        print("Load took", (t1-t0)*1000, "ms")

        assert torch.isclose(embeddings["x"], x).all() and all([torch.isclose(features[k], embeddings["features"][k]).all() for k in embeddings["features"] if not embeddings["features"][k] is None])
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        with torch.set_grad_enabled(True):
            batch = torch.empty(20, 3, res, res, device=device).normal_()
            targets = torch.empty(1, 21, res, res, device=device).normal_()
