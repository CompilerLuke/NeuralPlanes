...
class RansacMiningConf:
    ...
    top_k: int = 5
    area: float = 5
    scale_factor: float = 1.0 / 4


def mine_negatives(map: NeuralMap, floor_plane: int, rel_x0: torch.Tensor, rel_x1: torch.Tensor, camera: Camera,
                   negative_cameras: List[Camera], image: torch.Tensor, depths: torch.Tensor, conf: RansacMiningConf):
    device = image.device
    ...
    prob, weight, pos, features = score_pose(map, camera, image, depths, dense=True, plane_indices=plane_indices,
                                             rel_x0=rel_x0[None], rel_x1=rel_x1[None])
    ...
    if ref_score < 1e-3:
        print(f"Pose invalid for patch floor_plane={floor_plane} x0={rel_x0}, x1={rel_x1}")
        return False

    if not torch.isfinite(ref_score):
        score_pose(map, camera, image, depths, dense=True, plane_indices=plane_indices, rel_x0=rel_x0[None],
                   rel_x1=rel_x1[None])
        raise "Ref score not finite"
    ...
    height, width = int(ceil(u1 * plane_width) - floor(u0 * plane_width)), int(
        ceil(v1 * plane_height) - floor(v0 * plane_height))
    ...
    coords = torch.stack([u.flatten(), v.flatten(), torch.zeros_like(u).flatten()], dim=1)
    n_coords = coords.shape[0]

    def score_cell_per_feature(feature):
        # todo: avoid repeating feature and broadcast directly
        score, weight = score_cells(map, plane_indices.to(device), coords[None], feature[:, None].repeat(1, n_coords),
                                    torch.ones((n_coords), device=device), x0=rel_x0[None], x1=rel_x1[None])
        return score[0]  # get score for single plane
    ...
    def eval_model(new_cam, scale_depth=1.0):
        score, weight = score_pose(map, new_cam, image, scale_depth * depths, rel_x0=rel_x0[None], rel_x1=rel_x1[None],
                                   plane_indices=plane_indices)
        return score

    def move(cam, new_cam):
        cam = copy.copy(cam)
        cam.t = new_cam.t
        cam.R = new_cam.R
        return cam

    negatives = torch.zeros(conf.ransac_it + len(negative_cameras), device=device)
    negatives_count = len(negative_cameras)
    if negatives_count > 0:
        negatives[:negatives_count] = torch.stack([eval_model(move(camera, new_cam)) for new_cam in negative_cameras])  # todo: batch

    x0 = x0.to(device)
    un = un.to(device)
    vn = vn.to(device)
    rel_to_abs_coord = torch.stack([us_len, vs_len, torch.ones_like(us_len)]).to(device)
    camera_pos_plane_basis = torch.stack(
        [torch.dot(un, camera.t - x0), torch.dot(vn, camera.t - x0), torch.dot(normal, camera.t - x0)])
    ...
    for i in range(conf.ransac_it):
        ...
        # todo: use z coordinate from matching
        ...
        ref_pos = torch.stack([torch.einsum("j,ij->i", un, ref_pos - x0), torch.einsum("j,ij->i", vn, ref_pos - x0),
                               torch.einsum("j,ij->i", normal, ref_pos - x0)], dim=1) - camera_pos_plane_basis
        ...
        # print("Scoring pose")
        score = eval_model(new_cam, s)
        if not torch.isfinite(score):
            eval_model(new_cam, s)
    ...
    top_k_scores = top_k_scores[top_k_scores > 1e-3]
    ...
    if len(top_k_scores) > 0 and ref_score > 1e-3:
        loss = -(ref_score / top_k_scores.mean())
        #loss + loss_fn(1e-9 + (1 - 2e-9) * top_k_scores, torch.zeros(len(top_k_scores), device=device))
        return loss, ref_score, top_k_scores.mean()
    else:
        return False # torch.tensor(0.0,device=device), ref_score, 0.0