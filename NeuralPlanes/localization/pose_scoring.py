...
def score_cells(map: NeuralMap, plane_indices: torch.Tensor, coords: torch.Tensor, values: torch.Tensor, occupancy: torch.Tensor):
    ...
    coords = planes.coord_x0[plane_indices][:,None] + planes.coord_size[plane_indices][:,None] * coords
    ...
    alpha_cells = torch.nn.functional.grid_sample(map.atlas_alpha.reshape((1,n_components*n_features, atlas_height, atlas_width)), coords[None])[0]
    ...
    def score_cell(weight, alpha, mu, var, value, occupancy):
        ...
        log_prob = mixture.score_samples(occupancy, value, normalize=False)
...
def score_pose(map: NeuralMap, cam: Camera, image: torch.Tensor, depths: torch.Tensor, samples: int = 5, dense=False):
    ...
    plane_indices = torch.arange(len(planes))[visibility_mask]
    ...
    orig, dir = compute_ray(height,width,cam,device)
    pos = orig[:,:,None] + dir[:,:,None] * t_samples[:,:,:,None]

    us = planes.us[plane_indices]
    vs = planes.vs[plane_indices]
    us = us / torch.linalg.norm(us,dim=1)[:,None]
    vs = vs / torch.linalg.norm(vs,dim=1)[:,None]

    pos = pos.reshape((height*width*samples,3))

    coords = torch.stack([torch.einsum("ik,jk->ij", us, pos), torch.einsum("ik,jk->ij",vs,pos)], dim=2)
    ...
    occupancy = torch.full((height,width,samples,), 1.0) # / (height * width * samples))
    ...
    log_prob = (log_prob * weight).sum(dim=0) / (1e-9 + weight.sum(dim=0))
