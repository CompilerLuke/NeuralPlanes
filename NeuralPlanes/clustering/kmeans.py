...
from torch_kmeans.utils.distances import BaseDistance, CosineSimilarity
...
class SoftKMeans(KMeans):
    ...
    def _cluster(
        ...
        with torch.no_grad():
            for i in range(self.max_iter):
                ...
                # update
                centers = self._cluster_iter(x, centers, weights)
        ...
        with torch.enable_grad():
            centers[k_mask] = 0
            centers = self._cluster_iter(x, centers.detach().clone(), weights=weights)
            centers[k_mask] = 0
            dist = self._pairwise_distance(x, centers)
            dist = dist.clone()
    ...
    def _cluster_iter(self, x: Tensor, centers: Tensor, weights: Tensor) -> Tensor:
        ...
        c_assign = weights[:,None,:,None] * torch.softmax(self.temp * dist + self.eps, dim=-1)
