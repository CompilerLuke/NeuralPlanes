...
from NeuralPlanes.volume import alpha_blend, alpha_weight
...
def train_map(map_module: MapModule, image_loader, output_dir):
    ...
    #lr=3e-2, params=map_module.parameters())
    ...
    for epoch in range(num_epochs):
        for images, cams, corrs in tqdm.tqdm(image_loader):
            ...
            for i, (cam, (p2d, p3d)) in enumerate(zip(cams, corrs)):
                ...
                #print("P2D shape", p2d.shape, "P3D shape", p3d.shape, "Depth shape", depth.shape, "t indexed ", t_fine[i,:,p2d[:,1],p2d[:,0]].shape)
                ...
                #print("Depth range ", depth_based_density.min(), depth_based_density.max())
                ...
                # torch.einsum("ij,ij->j", torch.log(density_fine_p2), depth_based_density)) 
